    ...
    match op {
        Op::StoreRecord(store_record) => {
            if let Some(EntryType::App(AppEntryDef {
                ...
                zome_index,
                ..
            })) = store_record.record.action().entry_type()
            {
                ...
                if let RecordEntry::Present(entry) = store_record.record.entry() {
                        ...
                        EntryTypes::deserialize_from_type(*zome_index, *entry_index, &entry)?
                    {
                        ...
                        Ok(ValidateCallbackResult::Invalid(format!(
                            "No matching EntryTypes value for: {}/{}",
                            zome_index.0, entry_index.0
                        )))
                    }
                } else {
                    Ok(ValidateCallbackResult::Invalid(format!(
                        "Record with AppEntryDef was expected to have a Present(entry): {:?}",
                        store_record.record
                    )))
                }
...
fn validate_memory_update(
    _action: &Update,
    _memory: MemoryEntry,
) -> ExternResult<ValidateCallbackResult> {
    Ok(ValidateCallbackResult::Invalid(format!(
        "Memory entries cannot be updated"
    )))
}

fn validate_memory_delete(_action: &Delete) -> ExternResult<ValidateCallbackResult> {
    Ok(ValidateCallbackResult::Invalid(format!(
        "Memory entries cannot be deleted"
    )))
}
...
// Memory Block
//
fn validate_memory_block_create(
    _action: &Create,
    memory_block: MemoryBlockEntry,
) -> ExternResult<ValidateCallbackResult> {
    if memory_block.bytes.len() > 2_097_152 {
        return Ok(ValidateCallbackResult::Invalid(
            "MemoryBlockEntry cannot be larger than 2MB (2,097,152 bytes)".to_string(),
        ));
    } else {
        Ok(ValidateCallbackResult::Valid)
    }
...
fn validate_memory_block_update(
    _action: &Update,
    _memory_block: MemoryBlockEntry,
) -> ExternResult<ValidateCallbackResult> {
    Ok(ValidateCallbackResult::Invalid(format!(
        "MemoryBlock entries cannot be updated"
    )))
}

fn validate_memory_block_delete(_action: &Delete) -> ExternResult<ValidateCallbackResult> {
    Ok(ValidateCallbackResult::Invalid(format!(
        "MemoryBlock entries cannot be deleted"
    )))
}
